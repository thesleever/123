...
static CObjectSapper* GetSapperOnTeleporter(CObjectTeleporter* pTeleporter)
{
	if (!pTeleporter || !pTeleporter->HasSapper())
	{
		return nullptr;
	}

	CBaseEntity* pChild = pTeleporter->FirstMoveChild();
	while (pChild)
	{
		CObjectSapper* pSapper = dynamic_cast<CObjectSapper*>(pChild);
		if (pSapper)
		{
			return pSapper;
		}
...
void CObjectTeleporter::RecieveTeleportingPlayer(CTFPlayer* pTeleportingPlayer)
{
	...
	//CObjectSapper* pSapper = GetSapper(); // ОНО ПРОСТО ПРОВЕРЯЕТ НА САППЕР В РУКАХ АААААААА
	Vector newPosition;
	CObjectSapper* pSapper = GetSapperOnTeleporter(this);

	if (pSapper)
	{
		CALL_ATTRIB_HOOK_INT_ON_OTHER(pSapper->GetBuilder()->Weapon_GetWeaponByType(TF_WPN_TYPE_BUILDING), doRandomTeleport, sapper_random_teleport_position);
		Msg("obj_tele: %d", doRandomTeleport);
	}
